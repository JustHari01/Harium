...
import me.jellysquid.mods.lithium.common.compat.vs.LevelHolder;
import me.jellysquid.mods.lithium.common.compat.vs.VSCompat;
...
public abstract class PointOfInterestStorageMixin extends SerializingRegionBasedStorage<PointOfInterestSet>
        implements PointOfInterestStorageExtended, LevelHolder {

    // VS Compatibility: Store world reference for coordinate transforms
    @Unique
    @Nullable
    private World lithium$level = null;

    @Override
    @Nullable
    public World lithium$getLevel() {
        return this.lithium$level;
    }

    @Override
    public void lithium$setLevel(@Nullable World world) {
        this.lithium$level = world;
    }
    ...
    private Stream<PointOfInterest> withinSphereChunkSectionSortedStream(Predicate<RegistryEntry<PointOfInterestType>> predicate, BlockPos origin,
        ...
        // VS Compatibility: Use VSCompat for coordinate transforms when VS is loaded
        final World level = this.lithium$level;
        return stream.flatMap((Stream<PointOfInterestSet> setStream) -> setStream.flatMap(
                (PointOfInterestSet set) -> set.get(predicate, status)
                        .filter(point -> VSCompat.isWithinSphereRadius(level, origin, radiusSq, point.getPos()))
        ));
    }
    ...
    private ArrayList<PointOfInterest> withinSphereChunkSectionSorted(Predicate<RegistryEntry<PointOfInterestType>> predicate, BlockPos origin,
        ...
        Consumer<PointOfInterest> collector = point -> {
            if (VSCompat.isWithinSphereRadius(level, origin, radiusSq, point.getPos())) {
                points.add(point);
            }
        };
