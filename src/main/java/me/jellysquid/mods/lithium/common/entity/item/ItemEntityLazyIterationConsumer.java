...
/**
 * This LazyIterationConsumer collects item entities that the searching item
 * entity can merge with.
 * The merging operation itself is NOT performed, as this would cause
 * ConcurrentModificationException.
 * Instead, the merging operations are simulated to determine a list of item
 * entities the merging operation
...
public class ItemEntityLazyIterationConsumer implements LazyIterationConsumer<ItemEntity> {
    ...
    public NextIteration accept(ItemEntity otherItemEntity) {
        ...
        // Since we are not creating a copy of the list of entities, we must determine
        // the entities to interact with
        // before starting to modify any item entities, as this would otherwise
        // invalidate the iterator.
        // Dry run the merging logic to determine a small list of entities to merge
        // with:
        int receivedItemCount = predictReceivedItemCount(this.searchingEntity, this.stack, this.adjustedStackCount,
                otherItemEntity, otherItemEntity.getStack());
        if (receivedItemCount != 0) {
            // The item entity is mergeable, so add it to the list of entities to merge with
            // and adjust the stack count.
    ...
    /**
     * This method is a copies the merging logic from ItemEntity but without
     * applying the changes.
     * Here the number of items transferred between the two item entities is
     * calculated after
     * the two item entities have been determined to be mergeable.
     */
    private static int predictReceivedItemCount(ItemEntity thisEntity, ItemStack thisStack, int adjustedStackCount,
            ItemEntity otherEntity, ItemStack otherStack) {
        if (Objects.equals(((ItemEntityAccessor) thisEntity).lithium$getOwner(),
                ((ItemEntityAccessor) otherEntity).lithium$getOwner())
