...
public class ItemEntityCategorizingList extends ElementCategorizingList<ItemEntity, Item> {
    ...
    // If there are enough item entities in one category, divide the item entities
    // into 3 buckets:
    // Stacks that are more than 50% full can only merge with stacks that are less
    // than 50% full, etc.
    // Buckets: A B *
    ...
    public LazyIterationConsumer.NextIteration consumeForEntityStacking(ItemEntity searchingEntity,
            LazyIterationConsumer<ItemEntity> itemEntityConsumer) {
    ...
    void onElementSubcategorized(ItemEntity element, int index) {
        // Subcategorizing is based on the item stack count. The collection must be
        // updated whenever the stack count changes.
    ...
    private void updateSubcategoryAssignment(Item category, int index, boolean categoryA, boolean oldCategoryA,
            boolean categoryB, boolean oldCategoryB) {
    ...
    private void notifyAfterStackSwap(int index, ItemEntity element, ItemStack oldStack) {
        ...
        // Fix the stack subscription. We can reuse the subscriber object as it only
        // stores a reference to this list and the element.
        ...
        this.updateCategoryAndSubcategories(category, previousCategory, index, subCategoryA, prevSubCategoryA,
                subCategoryB, prevSubCategoryB);
    ...
    private void onAllElementsUnSubcategorized() {
        for (Reference2ReferenceMap.Entry<ItemEntity, ItemStackSubscriber> entry : this.subscribers
