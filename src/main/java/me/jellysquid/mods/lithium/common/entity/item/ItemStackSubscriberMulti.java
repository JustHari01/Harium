...
public class ItemStackSubscriberMulti implements ItemStackSubscriber {
    ...
    public ItemStackSubscriberMulti(ItemStackSubscriber subscriber1, int slot1, ItemStackSubscriber subscriber2,
            int slot2) {
    ...
    private ItemStackSubscriberMulti(ItemStackSubscriber[] subscribers, int[] indices) {
        ...
        if (subscribers.length != indices.length) {
            throw new IllegalArgumentException(
                    "Cannot create a multi-subscriber with different subscriber and slot lengths");
        }
