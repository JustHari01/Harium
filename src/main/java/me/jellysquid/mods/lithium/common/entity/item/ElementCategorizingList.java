...
public abstract class ElementCategorizingList<T, Category> extends AbstractList<T> {
    ...
    // This may require a lot of work for compatibility with the item entity
    // movement tracking and block entity sleeping
    @SafeVarargs
    ...
    @SuppressWarnings("unused")
    public LazyIterationConsumer.NextIteration consumeCategory(LazyIterationConsumer<T> elementConsumer,
            Category category) {
        IntArrayList categoryList = this.elementsByType.get(category);
        return this.consumeElements(elementConsumer, categoryList);
    }

    public LazyIterationConsumer.NextIteration consumeCategoryA(LazyIterationConsumer<T> elementConsumer,
            Category category) {
    ...
    public LazyIterationConsumer.NextIteration consumeCategoryB(LazyIterationConsumer<T> elementConsumer,
            Category category) {
    ...
    private LazyIterationConsumer.NextIteration consumeElements(LazyIterationConsumer<T> elementConsumer,
            IntArrayList categoryList) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            // The consumer must not modify the consumed element and or other elements in
            // the collection, or must return ABORT.
            LazyIterationConsumer.NextIteration next = elementConsumer.accept(element);
    ...
    public T set(int index, T element) {
        ...
        if (previous != element) {
            ...
            Category category = this.getCategory(element);
            // Fix the indices stored in elementsByType, elementsByTypeA and
            // elementsByTypeB.
            ...
            this.updateCategoryAndSubcategories(category, previousCategory, internalIndex, subCategoryA,
                    prevSubCategoryA, subCategoryB, prevSubCategoryB);
    ...
    void updateCategoryAndSubcategories(Category category, Category previousCategory, int internalIndex,
            boolean subCategoryA, boolean prevSubCategoryA, boolean subCategoryB, boolean prevSubCategoryB) {
        ...
        if (changedCategory) {
            ...
            binarySearchIndex = -(binarySearchIndex + 1); // Get insertion location according to
        ...
        if (previousCategoryList.size() >= CATEGORY_DOWNGRADE_THRESHOLD) {
            this.removeFromSubCategories(previousCategoryList, previousCategory, internalIndex,
                    prevSubCategoryA && changedA, prevSubCategoryB && changedB, changedCategory);
        }

        // Add to elementsByTypeA and elementsByTypeB
        this.addToSubCategories(categoryList, category, internalIndex, subCategoryA && changedA,
                subCategoryB && changedB, changedCategory);
    }

    void removeFromSubCategories(Category previousCategory, int internalIndex, boolean removeFromA, boolean removeFromB,
            @SuppressWarnings("SameParameterValue") boolean allowRemoval) {
        this.modCount++;
        this.removeFromSubCategories(this.elementsByType.get(previousCategory), previousCategory, internalIndex,
                removeFromA, removeFromB, allowRemoval);
    }

    private void removeFromSubCategories(IntArrayList previousCategoryList, Category previousCategory,
            int internalIndex, boolean removeFromA, boolean removeFromB, boolean allowRemoval) {
    ...
    void addToSubCategories(Category category, int internalIndex, boolean subCategoryA, boolean subCategoryB,
            @SuppressWarnings("SameParameterValue") boolean allowCreation) {
        this.modCount++;
        IntArrayList categoryList = this.elementsByType.computeIfAbsent(category, e -> new IntArrayList());
        this.addToSubCategories(categoryList, category, internalIndex, subCategoryA, subCategoryB, allowCreation);
    }

    private void addToSubCategories(IntArrayList categoryList, Category category, int internalIndex,
            boolean addToCategoryA, boolean addToCategoryB, boolean allowCreation) {
        if (categoryList.size() > CATEGORY_DOWNGRADE_THRESHOLD) {
            IntArrayList categoryListA = this.elementsByTypeA.get(category);
            if (categoryListA != null) {
                if (addToCategoryA) {
