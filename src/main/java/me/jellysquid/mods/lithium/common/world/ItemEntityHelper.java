...
public class ItemEntityHelper {
    ...
    @NotNull
    public static BucketedList<ItemEntity> getItemEntityBucketedList(Hopper hopper, double xOffset, double yOffset,
            double zOffset, List<ItemEntity> nearbyEntities) {
    ...
    public static void consumeItemEntitiesForMerge(SectionedEntityCache<Entity> cache, ItemEntity searchingItemEntity,
            Box box, LazyIterationConsumer<ItemEntity> itemEntityConsumer) {
        cache.forEachInBox(box, section -> {
            ...
            if (itemEntities.size() > ItemEntityCategorizingList.UPGRADE_THRESHOLD
                    && itemEntities instanceof ArrayList<ItemEntity>) {
                itemEntities = internalEntityList.lithium$replaceCollectionAndGet(ItemEntity.class,
                        ItemEntityCategorizingList::wrapDelegate);
            }
